...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        L = len(reward_list)
        gamma_list = np.array([self.gamma ** t for t in range(0, L)])
        dot = sum(reward_list * gamma_list)
        return np.array([dot for _ in range(len(rewards))])
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        # we will use cum sum here but we need to be a little smart about it
        # I think this works sort of
        # reverse but cross check
        reward_list = np.array(rewards)
        L = len(reward_list)
        gamma_list = np.array([self.gamma ** t for t in range(0, L)])
        dot_list = reward_list * gamma_list
        cum_sum_rev = np.cumsum(dot_list[::-1])
        output = cum_sum_rev[::-1]
        return output / gamma_list
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            eps = 10 ** -8
            advantages = (advantages - np.mean(advantages))/(eps + np.std(advantages))
